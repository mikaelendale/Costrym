# CostValueAlignerAgent Documentation
...
- **Persona (if LLM agent):** Cost-to-Value Alignment (CVA) Orchestrator. Purely orchestration—never performs its own analysis, only manages sub-agent invocation and output sanitization.

## 2. Behavior Summary

The agent acts as an orchestrator, sequentially invoking the `ValueMapper` and then the `SmartReducer` sub-agents. It passes the raw user input to `ValueMapper`, processes the result, and then passes the enriched output to `SmartReducer`. The final output is sanitized to ensure it is a single, valid JSON object, with all non-JSON commentary stripped. The agent never performs its own analysis or transformation beyond orchestration and output validation. All orchestration logic and output constraints are defined in `resources/prompts/cost_value_aligner_agent/default.blade.php`.
...
- **Sub-agents:**
    ...
    - `SmartReducer` (class: `App\Agents\SmartReducer`): - **Purpose:** Receives the enriched portfolio from `ValueMapper` and filters out value-negative items. Produces the final JSON object of tasks or actions that are value-aligned and ready for execution. - **Invocation:** Always called second, after `ValueMapper`.

- **Tools:**
    - `DelegateToSubAgentTool` (class: `Vizra\VizraADK\Tools\DelegateToSubAgentTool`): - **Purpose:** Provides the mechanism for this agent to delegate execution to its sub-agents in a controlled, sequential manner. Ensures that sub-agent calls are managed and their outputs are properly captured and passed along the pipeline.
...
- All orchestration and output constraints are defined in `resources/prompts/cost_value_aligner_agent/default.blade.php`.
