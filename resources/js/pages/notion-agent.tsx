...
export default function NotionAgent() {
    const { auth } = usePage<SharedData>().props;
    const [messages, setMessages] = useState<Message[]>([]);
    const [input, setInput] = useState('');
    const [isLoading, setIsLoading] = useState(false);
    const [sessionId] = useState(() => `notion_${auth.user?.id}_${Date.now()}`);
    ...
    const handleSend = async () => {
        ...
        try {
            const response = await fetch('/notion-agent/chat', {
                method: 'POST',
                headers: {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Notion Agent" />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="flex flex-col gap-2">
                    <h1 className="text-2xl font-bold">Notion Agent</h1>
                    <p className="text-muted-foreground">
                        Chat with your Notion AI agent. Ask it to create pages, update databases, search content, and more.
                    </p>
                </div>
                        ...
                        {messages.length === 0 ? (
                            <div className="flex h-full items-center justify-center text-center text-muted-foreground">
                                <div className="space-y-2">
                                    ...
                                    <p className="text-lg font-medium">Start chatting with your Notion agent</p>
                                    <p className="text-sm">Try asking: &quot;What Notion actions can you use?&quot;</p>
                                </div>
                            </div>
                        ) : (
