...
class FirstTimeCostAnalysisJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        Cache::put("analysis_status_{$this->userId}", [
            'status' => 'started',
        ...
        if ($existingAnalysis) {
            ...
            // Clear the status cache and lock since we're not running
            Cache::forget("analysis_status_{$this->userId}");
            Cache::forget("analysis_dispatch_{$this->userId}");

            return;
        }

        // Clear the dispatch lock since we're starting
        Cache::forget("analysis_dispatch_{$this->userId}");
        ...
        try {
            // Helper to update cache status
            $updateStatus = function ($status, $message, $data = []) {
                Cache::put("analysis_status_{$this->userId}", [
                    'status' => $status,
                    'message' => $message,
                    'data' => $data,
            ...
            $updateStatus('analyzing', 'Step 1/8: Cost Decomposition', [
                'current_step' => 1,
                'total_steps' => 8,
            ]);
            ...
            $updateStatus('analyzing', 'Step 2/8: Benchmark Analysis', [
                'current_step' => 2,
                'total_steps' => 8,
            ]);
            ...
            $updateStatus('analyzing', 'Step 3/8: Cost Efficiency Analysis', [
                'current_step' => 3,
                'total_steps' => 8,
            ]);
            ...
            $updateStatus('analyzing', 'Step 4/8: Root Cause Analysis', [
                'current_step' => 4,
                'total_steps' => 8,
            ]);
            try {
            ...
            $updateStatus('analyzing', 'Step 5/8: Solution Generation', [
                'current_step' => 5,
                'total_steps' => 8,
            ]);
            ...
            $updateStatus('analyzing', 'Step 6/8: Impact Simulation', [
                'current_step' => 6,
                'total_steps' => 8,
            ]);
            ...
            $updateStatus('analyzing', 'Step 7/8: Value Mapping', [
                'current_step' => 7,
                'total_steps' => 8,
            ]);
            ...
            $updateStatus('analyzing', 'Step 8/8: Final Recommendations', [
                'current_step' => 8,
                'total_steps' => 8,
            ]);
            ...
            // Clear the dispatch lock since we completed
            Cache::forget("analysis_dispatch_{$this->userId}");

            // Note: MasterOrchestratorJob can be scheduled here when it's created
            ...
            // Clear the dispatch lock since we failed
            Cache::forget("analysis_dispatch_{$this->userId}");
